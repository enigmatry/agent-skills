...
"scripts": {
  "build": "react-scripts build"  // Automatically minifies
}

// Check for custom webpack config (eject)
// Verify production mode and optimization settings
```
...
module.exports = {
  ...
  swcMinify: true,  // Use SWC for faster minification (recommended)
  
  // Check for disabled minification (RED FLAG)
  webpack: (config, { dev }) => {
    if (!dev) {
      // Verify minification is not disabled
      // config.optimization.minimize should be true
    }
    return config;
  }
}
...
export default defineConfig({
  build: {
    minify: 'terser',  // or 'esbuild' - MUST be set for production
    terserOptions: {
      compress: {
...
module.exports = {
  mode: 'production',  // MUST be 'production' for production builds
  optimization: {
    minimize: true,    // MUST be true
    minimizer: [
      new TerserPlugin({
        terserOptions: {
          compress: {
...
// vue.config.js
module.exports = {
  productionSourceMap: false,  // SHOULD be false
...
**Vite (Vue 3):**
- Same configuration as React Vite (see above)
- Check `vite.config.js` for `build.minify` option
...
**Other frameworks:**
- Check for HTML minification plugins/configuration
...
**Suggestions for automated verification:**
...
1. Add build verification script:
   ...
   const hasSourceMaps = fs.readdirSync('./dist')
     .some(file => file.endsWith('.map'));
   
   if (hasSourceMaps) {
     console.error('ERROR: Source maps found in production build!');
     process.exit(1);
   }
   ```
...
3. File size checks:
   ...
   - Alert if bundles are unusually large (possible unminified code)
